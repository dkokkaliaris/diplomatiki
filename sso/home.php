<?php
  ...
  if (isset($_REQUEST['logout'])) {
    ...
    if ($row2->email_counter==0) {//Αν δεν υπαρχει ο χρηστης στην βαση τον προσθετω.
        ...
        // ελέγχει τι τυπος ειναι ο χρηστης και αν ειναι καθηγητης βάζει το 2 αλλιώς το 3 για φοιτητη
        ...
        $params = array(':username' => $username, ':email' => $email, ':password' => $password, ':type' => $type, ':activated' => $activated, ':last_name' => $last_name, ':first_name' => $first_name, ':telephone' => $telephone, ':aem' => $aem);

        $sql = 'INSERT INTO dk_users (username, email, password, type, activated, last_name, first_name, telephone, aem) VALUES (:username, :email, :password, :type, :activated, :last_name, :first_name, :telephone, :aem)';
